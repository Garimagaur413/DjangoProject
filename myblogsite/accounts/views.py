...
def edit_profile_view(request):
    if request.method=='POST':
        ...
        profile.save()
        messages.success(request,"profile details are updates successfully")
...
def login_view(request):
    if request.method=='POST':
        user=authenticate( username=request.POST['emailid'],
...
def profile_view(request):
    return render(request,"viewprofile.html",{'user':request.user})
@login_required(login_url='login')
